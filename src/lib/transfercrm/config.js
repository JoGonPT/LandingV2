...
exports.getTransferCrmConfig = getTransferCrmConfig;
exports.buildTransferCrmAuthHeaders = buildTransferCrmAuthHeaders;
const DEFAULT_TIMEOUT_MS = 12000;
function parseAuthMode(raw) {
    const v = raw?.trim().toLowerCase();
    if (v === "x_api_key" || v === "x-api-key" || v === "api_key" || v === "apikey")
        return "x_api_key";
    if (v === "authorization_api_key" || v === "apikey_prefix" || v === "sanctum")
        return "authorization_api_key";
    if (v === "basic")
        return "basic";
    return "bearer";
}
function getTransferCrmConfig() {
...
function buildTransferCrmAuthHeaders(auth) {
    switch (auth.mode) {
        case "bearer":
