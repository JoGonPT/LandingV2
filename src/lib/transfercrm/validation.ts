...
export function validateBookingPayload(
  ...
  if (candidate.quotedPrice !== undefined) {
    ...
    const qp = candidate.quotedPrice as { amount?: unknown; currency?: unknown };
  ...
  const normalized: BookingPayload = {
    ...
    ...(candidate.quotedPrice
      ? {
          quotedPrice: {
            amount: (candidate.quotedPrice as { amount: number }).amount,
            currency: (candidate.quotedPrice as { currency: string }).currency.trim(),
          },
        }
      : {}),
    contact: {
